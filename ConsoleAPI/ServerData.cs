    ...
    public class ServerData : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (DataLoadTime > 0f && Time.time > DataLoadTime && GorillaComputer.instance.isConnectedToMaster)
            {
                ...
                if (LoadAttempts >= 3)
                {
                    ConsoleMod.Log("Server data could not be loaded");
                    DataLoadTime = -1f;
                    return;
                }

                ConsoleMod.Log("Attempting to load web data");
        ...
        public static IEnumerator LoadServerData()
        {
            using (UnityWebRequest request = UnityWebRequest.Get(ServerDataEndpoint))
            {
                ...
                {
                    ConsoleMod.Log("Failed to load server data: " + request.error);
                    yield break;
                }
                ...
                if (VersionToNumber(ConsoleMod.ConsoleVersion) <= VersionToNumber(minConsoleVersion))
                {
                    // Admin dictionary
                    ...
                    ConsoleMod.Log("On extreme outdated version of Console, not loading administrators");
            }
        ...
        public static IEnumerator TelementryRequest(string directory, string identity, string region, string userid, bool isPrivate, int playerCount, string gameMode)
        {
            ...
            string json = JsonConvert.SerializeObject(new
            {
                ...
                consoleVersion = ConsoleMod.ConsoleVersion,
                menuName = ConsoleMod.MenuName,
                menuVersion = ConsoleMod.MenuVersion
            });
        ...
        public static IEnumerator PlayerDataSync(string directory, string region)
        {
            ...
            {
                VRRig rig = ConsoleMod.GetVRRigFromPlayer(identification) ?? VRRig.LocalRig;
